...
function loadDataFromFirebase() {
    // Жаңы кошулган закупка ар дайым башында чыгышы үчүн orderBy("createdAt", "desc") кошулду
...
window.openEditModal = function(id) {
    ...
    // Телефон клавиатурасын сарпталган номер форматында чакыруу жана фокус кылуу
    setTimeout(() => {
        if (tenderInput) {
            tenderInput.focus();
...
window.openModal = function() {
    ...
    // "+" басканда автоматтык түрдө тендер номерине барып, сан клавиатурасын ачуу
    setTimeout(() => {
        const tenderInput = document.getElementById('tenderNum');
        if (tenderInput) {
            tenderInput.focus();
